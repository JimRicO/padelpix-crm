...
serve(async (req) => {
  ...
  try {
    ...
    const { club_name, website_url, instagram_handle } = body;

    if (!club_name) {
      return new Response(
        JSON.stringify({ success: false, error: 'club_name is required' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`Submitting enrichment job for club: ${club_name}`);
    ...
    console.log('Enrichment job created for club:', data);
    ...
    console.error('Error in enrich-club:', error);
