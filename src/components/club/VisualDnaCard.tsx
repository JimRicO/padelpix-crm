...
import { ChevronDown, ChevronRight, Eye, Palette, Camera, Layout, Stamp, PieChart, MessageSquare, Target, Code, Image, X, ArrowLeft, ArrowRight } from 'lucide-react';
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[95vw] max-h-[95vh] p-0 bg-black/95 border-none">
        <div className="relative w-full h-[90vh] flex items-center justify-center">
          ...
          {/* Image counter */}
          <div className="absolute top-4 left-4 z-50 text-white/80 text-sm font-medium bg-black/50 px-3 py-1 rounded-full">
...
export function VisualDnaCard({ club }: VisualDnaCardProps) {
  ...
  const mediaUrls = club.visual_dna_media_urls as string[] | null;
  ...
  const navigateLightbox = useCallback((direction: 'prev' | 'next') => {
    if (!mediaUrls) return;
    if (direction === 'prev') {
      setLightboxIndex((prev) => (prev > 0 ? prev - 1 : mediaUrls.length - 1));
    } else {
      setLightboxIndex((prev) => (prev < mediaUrls.length - 1 ? prev + 1 : 0));
    }
  }, [mediaUrls]);
        ...
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          ...
          <CollapsibleContent>
            <div className="mt-4 space-y-6 p-4 rounded-lg neu-pressed">
              ...
              {/* Section: Instagram Media Gallery */}
              {mediaUrls && mediaUrls.length > 0 && (
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-2">
                    <Image className="w-4 h-4" /> Instagram Media ({mediaUrls.length})
                  </h4>
                    ...
                    <div className="flex gap-2 pb-2">
                      {mediaUrls.map((url, i) => (
                        <button
                          ...
                          className="flex-shrink-0 w-32 h-32 rounded-lg overflow-hidden border border-border hover:ring-2 hover:ring-primary transition-all cursor-pointer"
                        >
                          <img
                            src={url}
                ...
                <CollapsibleContent>
                  <div className="mt-2 p-3 rounded bg-muted/50 overflow-auto max-h-64">
                    <pre className="text-xs font-mono">
                      {JSON.stringify({ visual_dna: visualDna, voice_dna: voiceDna, ctlt_matches: ctltMatches, media_urls: mediaUrls }, null, 2)}
                    </pre>
                  </div>
                </CollapsibleContent>
              </Collapsible>
