...
interface CreatePersonFromOrgKeyPeopleDialogProps {
  open: boolean;
...
export function CreatePersonFromOrgKeyPeopleDialog({
  open,
  onOpenChange,
  keyPerson,
  organizationName,
}: CreatePersonFromOrgKeyPeopleDialogProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Step 2: Link the person to the organization
      await createLink.mutateAsync({
        person_id: newPerson.id,
        link_type: 'ownership_group',
        ownership_group_name: organizationName,
        role_at_entity: data.role || null,
        is_primary: true,
      });

      toast.success(`Created ${data.full_name} and linked to ${organizationName}`);
      reset();
      onOpenChange(false);
    } catch (error) {
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="flex items-center gap-1.5">
            ...
            Will be linked to {organizationName}
          </DialogDescription>
        </DialogHeader>
